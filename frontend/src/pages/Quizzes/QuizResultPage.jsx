...
const QuizResultPage = () => {
  ...
  const correctAnswers = detailedResults.filter((result) => result.isCorrect);
  const incorrectAnswers = totalQuestions - correctAnswers.length;
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8">
        <div className="text-center space-y-6">
          <div className="inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 shadow-lg shadow-emerald-500/25 ">
            <Trophy className="size-7 text-emerald-600" strokeWidth={2} />
          </div>
          ...
          <div>
            ...
            <div
              className={`inline-flex text-5xl font-bold bg-linear-to-r ${getScoreColor(score)} bg-clip-text text-transparent mb-2`}
            >
              {score}%
            </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-2">
          <BookOpen strokeWidth={2} className="size-5 text-slate-600" />
        ...
        {detailedResults.map((result, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white/80 backdrop-blur-xl  border-2 border-slate-200 rounded-2xl px-6 shadow-lg shadow-slate-200/50 py-4"
            >
              ...
              {result.explanation && (
                <div className="p-4 bg-linear-to-br from-slate-50 to-slate-100/50 border border-slate-200 rounded-xl">
                  <div className="flex items-start gap-3">
                    <div className="shrink-0 size-8 rounded-lg bg-slate-200 flex items-center justify-center mt-0.5">
                      <BookOpen
                        strokeWidth={2}
        ...
        <Link to={`/documents/${quiz.document._id}`}>
          <button className="group relative px-8 h-12 bg-linear-to-r  from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden cursor-pointer">
            <span className="relative z-10 flex items-center gap-2">
              <ArrowLeft
                strokeWidth={2.5}
                className="size-4 group-hover:-translate-x-1 transition-all duration-200"
              />{" "}
              Return to Document
            </span>
